...
const Auth = () => {
  ...
  const handleAuth = async (e) => {
    ...
    try {
      ...
      if (err.message?.includes('fetch') || err.name === 'TypeError') {
        setErrorMessage(
          'Cannot reach the server. The database may be paused. Please try again later or contact support.'
        );
      } else {
  ...
  // Detect if error is a connectivity issue
    ...
    <div className="flex-center" style={{ minHeight: '100vh', background: 'var(--background)', padding: '20px' }}>
      ...
      >
        ...
        {errorMessage && (
          ...
          >
            {isNetworkError && <WifiOff size={16} style={{ flexShrink: 0, marginTop: '2px' }} />}
        ...
        <form onSubmit={handleAuth} style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
          ...
          <button
            ...
            disabled={loading}
            style={{ width: '100%', justifyContent: 'center', marginTop: '10px', height: '50px' }}
          >
            {loading
              ? 'Processing...'
              : isSignUp
